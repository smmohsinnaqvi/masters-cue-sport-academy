...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      { title: `Supervisor Dashboard | ${ACADEMY.name}` },
      {
        name: "description",
        content:
          "Monitor today's table queue, verify walk-in customers and block tables for maintenance or tournaments.",
      },
      { property: "og:title", content: `Supervisor Dashboard | ${ACADEMY.name}` },
      {
        property: "og:description",
        content: "Live booking queue, verification and bulk table blocking for floor supervisors.",
      },
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 pb-20 pt-10 sm:px-6 lg:px-8">
      ...
      <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
        Today's queue across all eight tables — who booked, on which table, and whether they have
        been verified at reception.
      </p>
      ...
      <Card className="mt-5 border-border bg-surface">
        ...
        <CardContent className="space-y-5 p-5">
          ...
          <Button
            type="button"
            onClick={applyBlock}
            disabled={blockTables.length === 0}
            className="min-h-12 w-full sm:w-auto"
          >
            Block {blockTables.length || "no"} table{blockTables.length === 1 ? "" : "s"} on{" "}
            {selectedDate.day}
          </Button>
        </CardContent>
      </Card>
    ...
    <div className="rounded-lg border border-border bg-surface p-4">
      ...
      <p
        className={
          ...
          (tone === "felt" ? "text-felt" : tone === "warning" ? "text-warning" : "text-foreground")
        }
      >
        {value}
      </p>
