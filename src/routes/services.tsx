...
export const Route = createFileRoute("/services")({
  head: () => ({
    meta: [
      { title: `Services & Facilities | ${ACADEMY.name}` },
      {
        name: "description",
        content:
          "Table rates, coaching programmes from beginner to pro, cafe menu, pro shop and cue lockers at Masters Cue Sport Academy.",
      },
      { property: "og:title", content: `Services & Facilities | ${ACADEMY.name}` },
      {
        property: "og:description",
        content: "Table rates, coaching packages, cafe menu and pro shop services.",
      },
          ...
          <Card key={pkg.id} className="border-border bg-surface">
            ...
            <CardContent className="p-5 pt-0">
              ...
              <Button asChild className="mt-5 min-h-12 w-full">
                <Link to="/booking">Book a coaching table</Link>
              </Button>
            </CardContent>
          </Card>
        ))}
